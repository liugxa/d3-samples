function ExitedRunningProgressBar(context, smtTime, stTime, etTime){
	...
	this.getProgressers = function(){
		...
		r.push(new Progresser(this.context, this.stTime, this.etTime, x2, y2, COLOR_EXITED));
	...
	this.getTimers = function(){
		...
		if(this.stTime){
			...
			var x2 = x1 + s2; 
			var y2 = y1;
			r.push(new Timer(this.context, "ST", this.stTime, x2, y2, COLOR_RED));
		}
		...
		if(this.etTime){
			...
			var x3 = x1 + s3; 
			var y3 = y2;
			r.push(new Timer(this.context, "ET", this.etTime, x3, y3, COLOR_RED));
		}
		return r;
	}
}